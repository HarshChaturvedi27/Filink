...
export default function Hero(){
    ...
    const handleUpload = async (files: FileList, folderPath?: string) => {
        ...
        try {
            ...
            const results = await uploadToS3Server(files, folderPath, (progress) => {
            ...
            const successfulUploads = results
                ...
                .map((result, index) => ({
                    name: result.isZip ? `${result.originalName} (${result.fileCount} files)` : files[index].name,
    ...
    const handleStartUpload = () => {
        ...
        const folderPath = selectedFiles.length > 1 ? `folder-${Date.now()}` : undefined
                    ...
                    {isUploading ? (
                        ...
                        <>
                            <div className="text-center mb-6">
                                ...
                                <p className="text-sm text-gray-500 mb-4">Select single or multiple files</p>
                            </div>
                            
                            <button
                                onClick={() => fileInputRef.current?.click()}
                                className="px-6 py-2 border border-dashed text-white rounded-lg hover:bg-blue-600 transition-colors"
                            >
                                Choose Files
                            </button>
                    ...
                    <div className="mt-8">
                        ...
                        <div className="space-y-2">
                            {uploadedFiles.map((file, index) => (
                                <div key={index} className="flex items-center justify-between p-3 border border-dashed rounded-lg">
                                    <div className="flex-1">
                                        <p className="font-medium">{file.name}</p>
                                        <p className="text-sm text-gray-500">
                                            {file.isZip ? `Zip file containing ${file.fileCount} files` : 'Click download to save file'}
                                        </p>
                                    </div>
                                    <div className="flex gap-2">
                                        ...
                                        <button
                                            ...
                                            className="px-4 py-2 border border-dashed text-white rounded-lg hover:bg-blue-600 transition-colors"
                                        >
                                            Copy Link
                                        </button>
                                    </div>
